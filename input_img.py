...
## THIS SOURCE CODE BUILDS THE HIGH DIMENSIONAL INPUT IMAGE/DESCRIPTOR USING ALREADY PREPROCESSED FOURIER-TRANSFORMED DATA FOR 27,587 ABX3 PEROVSKITES FROM OQMD.

## The input features are broadly separated into Direct Space Features (DSF) and Periodic Reciprocal Features (PRF) 

## Due to the large sizes of the data files, the complete data (as uploaded on GITHUB) is split and compressed into 3 different CSV files: data_1, data_2 and data_3. 
## Extract all files and run the codes, by ensuring that the complete data files and the source code are located in the same directory.
